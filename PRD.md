...
One shared codebase, board selected via **compile-time build environment** (`-DBOARD_BUILD_A` / `-DBOARD_BUILD_B` flags in `platformio.ini`), **not** a runtime NVS setting. This was confirmed during implementation: `display_factory.cpp` uses the build flag as the authoritative selector, and the NVS `boardTarget` default is seeded from the build flag at first boot. The portal Advanced settings for board selection (Section 8.7) were designed before this was settled — the portal option remains in scope but acts as a user-visible label for the compile target, not a live-switching mechanism. Shared logic (modes, portal, NVS schema, data fetching/caching, WiFi handling) is board-agnostic. Only the **hardware abstraction layer** (Section 2) and **rendering layer** (Section 6) branch by board.
...
Per the Claude Design mockup (v3, approved layout reference): header bar with date/time/market status, a 3-column index summary (S&P 500 / NASDAQ / DOW), and a data table of tracked tickers with six columns: TICKER, FUND, PRICE, DAY CHG, DAY H/L, and 15 DAY TREND.

**Index panels** each show the index price, day change, and a rolling 6-hour bar chart (6 hourly samples, oldest→newest) flanked by "H [max]" and "L [min]  6hrs" labels. The H/L values derive from the rolling history min/max, not a separate day-high/low API field.

**15 DAY TREND column** (replaces the former 52-week range bar): a 15-bar sparkline showing ~5 trading days of price history at 2-hour intervals, oldest left → newest right. Bar heights are normalized to each ticker's own min/max with a 12.5% floor so the lowest bar remains visible. Bars are drawn with `fillRect` (not Unicode block characters) for pixel-exact rendering on the ePaper display. Fetched via `/time_series?interval=2h&outputsize=15` — 1 additional credit per ticker per refresh cycle.
...
1. ~~Confirm exact GxEPD2 class names for both Board A display panel candidates~~ **Resolved:** `GxEPD2_750_T7` (GDEY075T7 / UC8179) confirmed working on both Board A and Board B. The panel-variant toggle (Section 5.2) remains as a fallback escape hatch for panels using the alternate EK79655 variant (`GxEPD2_750`).
...
3. ~~Confirm Twelve Data's actual free-tier field availability matches the Section 6.1 field set (day high/low, 52-week range)~~ **Resolved:** `/quote` endpoint returns `high`, `low`, `percent_change`, `close`, `previous_close` — all fields needed. The 52-week range field is no longer used (replaced by sparklines). `/time_series` confirmed available on free tier at `1h` and `2h` intervals. No tier restrictions; costs 1 credit per symbol per call.
...
5. ~~Finalize BOOT-hold duration for entering Config Mode~~ **Resolved:** confirmed 4s (Section 2.4).
6. Finalize exact backoff curve for WiFi retry after failures (Section 4.3)
7. Confirm field-set parity between Twelve Data and Finnhub responses is close enough that switching providers via the portal doesn't silently break parts of the Market Overview layout — Finnhub's `fetchTimeSeries` is currently a stub (returns zeros, `hasHistory = false`), so sparklines will not render when using Finnhub. Either implement Finnhub time series support or document this as a known limitation of the Finnhub provider.
8. ~~**Verify GxEPD2 actually has mature/working support for the reTerminal E1001 (Board B) specifically**~~ **Resolved:** GxEPD2 confirmed working on Board B (E1001). Both boards running the same driver class (`GxEPD2_750_T7`), code-sharing goal achieved.
